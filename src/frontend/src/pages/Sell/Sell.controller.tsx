...
export const Sell = ({ transactionPending, setTransactionPendingCallback }: SellProp) => {
    ...
    if (contract && accountPkh) {
      ...
      try {
        ...
        if (tokensOwnedFromStorage) {
          const tokenIds: number[] = tokensOwnedFromStorage.map((token: { c: any[] }) => token.c[0])
  ...
  const sellTokenCallback = React.useCallback(
    ({ token_id, price }: SellToken) => {
      return (contract as any).methods.sellLand(price * 1000000, token_id).send()
    },
    [contract],
  )
          ...
          {ready ? (
            <>
              {myTokens && myTokens.length > 0 ? (
                ...
                <div>
                  {loading ? <Message>Loading lands... Please wait.</Message> : <Message>No land available</Message>}
                </div>
              )}
            </>
          ) : (
