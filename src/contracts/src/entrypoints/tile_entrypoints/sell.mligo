...
(**
Put the tile on sale in the "on_sale" list and add this contract as an operator for this token
...
let sell (sell_params, storage : sell_param * nft_token_storage) : (operation  list) * nft_token_storage =
    ...
    then (failwith("Only the owner of a tile can sell it") : (operation  list) * nft_token_storage)
    else
        let tile : tile = get_tile_from_tiles(sell_params.token_id, storage.market.tiles) in
            if tile.onSale then
                (failwith("This tile is already on sale"): (operation  list) * nft_token_storage)
            else
                ...
                let tiles_with_updated_tile: tiles =  update_price_on_sale_flag_for_a_tile(sell_params.token_id, true, sell_params.price, storage.market.tiles) in
                ([] : operation list), { storage with market = { storage.market with sales = sales_with_new_sale; tiles = tiles_with_updated_tile }; operators = operators_with_contract_as_operator }